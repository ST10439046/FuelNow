...
import { createOrder, MOCK_USER } from '../../services/mockApi';
...
export default function OrderReviewScreen({ navigation, route }: Props) {
  ...
  const address = MOCK_USER.savedAddresses.find((a) => a.id === deliveryAddressId)
    ?? MOCK_USER.savedAddresses[0];
  const pm = MOCK_USER.paymentMethods.find((p) => p.id === paymentMethodId)
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      navigation.navigate('OrderPlaced', { orderId: order.id });
    } catch (e: any) {

...
import { postSOS } from '../../services/mockApi';
...
export default function SOSScreen({ navigation }: Props) {
  ...
  function handleSOSPress() {
    ...
    // 1200 ms "sending" animation, then call API
    setTimeout(async () => {
      try {
        const result = await postSOS('drv_001', { lat: -29.799, lng: 31.034 });
        setSosResult({ referenceNumber: result.referenceNumber, message: result.message });
      } catch (_) {
        setSosResult({
